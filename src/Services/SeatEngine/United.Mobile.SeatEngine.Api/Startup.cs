...
namespace United.Mobile.SeatEngine.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddTransient<ISeatEngineBusiness, SeatEngineBusiness>();
        ...
        public void ConfigureContainer(ContainerBuilder container)
        {
            ...
            container.Register(c => new ResilientClient(Configuration.GetSection("OnTimePerformanceInfoClient").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("OnTimePerformanceServiceKey");
            container.RegisterType<OnTimePerformanceInfoService>().As<IOnTimePerformanceInfoService>().WithAttributeFiltering();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IApplicationEnricher applicationEnricher, IFeatureSettings featureSettings, IHostApplicationLifetime applicationLifetime)
        {
            ...
            app.MapWhen(context => string.IsNullOrEmpty(context.Request.Path) || string.Equals("/", context.Request.Path), appBuilder =>
            {
                ...
                {
                    await context.Response.WriteAsync("Welcome from SeatEngine Microservice");
                });
            });
        ...
        private async Task OnStart(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.LoadFeatureSettings(United.Mobile.Model.Common.ServiceNames.SEATENGINE.ToString());
            }
            catch (Exception) { }
        ...
        private async Task OnShutDown(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.DeleteContainerIPAdress(United.Mobile.Model.Common.ServiceNames.SEATENGINE.ToString(), StaticDataLoader._ipAddress);
            }
            catch { }
        }
    }

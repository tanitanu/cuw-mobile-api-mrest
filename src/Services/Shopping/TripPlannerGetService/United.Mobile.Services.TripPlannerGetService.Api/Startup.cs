...
namespace United.Mobile.Services.TripPlannerGetService.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddScoped<ITripPlannerGetServiceBusiness, TripPlannerGetServiceBusiness>();
            ...
            services.AddTransient<ICMSContentService, CMSContentService>();
        ...
        public void ConfigureContainer(ContainerBuilder container)
        {
            ...
            container.Register(c => new ResilientClient(Configuration.GetSection("TravelPlannerServiceClient").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("TravelPlannerServiceClientKey");
            container.RegisterType<TravelPlannerService>().As<ITravelPlannerService>().WithAttributeFiltering();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IApplicationEnricher applicationEnricher, IFeatureSettings featureSettings, IHostApplicationLifetime applicationLifetime)
        {
            ...
            app.MapWhen(context => string.IsNullOrEmpty(context.Request.Path) || string.Equals("/", context.Request.Path), appBuilder =>
            {
                ...
                {
                    await context.Response.WriteAsync("Welcome from TripPlannerGetService Microservice");
                });
            });
        ...
        private async Task OnStart(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.LoadFeatureSettings(United.Mobile.Model.Common.ServiceNames.TRIPPLANNERGETSERVICE.ToString());
            }
            catch (Exception) { }
        ...
        private async Task OnShutDown(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.DeleteContainerIPAdress(United.Mobile.Model.Common.ServiceNames.TRIPPLANNERGETSERVICE.ToString(), StaticDataLoader._ipAddress);
            }
            catch { }
        }
    }
